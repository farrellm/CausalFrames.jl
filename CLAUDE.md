...
- `src/frame.jl` — `CausalFrame{T}`: opaque, backed by a single DataFrame;
  invariants checked in the inner constructor; Tables.jl interface
...
- `src/pipeline.jl` — `CausalPipeline` (lazy `Context -> iterator of
  DataFrame chunks`), `load` (drains into one frame — the only operation
  that materializes the whole window), `stream` (one frame per chunk)
...
- Never expose the backing DataFrame of a `CausalFrame`; `DataFrame(frame)`
